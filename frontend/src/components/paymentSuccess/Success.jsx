...
export const Success = () => {
  ...
  const drawInvoiceContent = (doc, data, margin, startY) => {
    ...
    data.products?.forEach((item) => {
      ...
      doc.text(Number(item.unitPrice).toFixed(2), 420, y);
            ...
            <div className="success-body">
              <div className="info-grid">
                {invoiceData.products?.map((p, idx) => (
                  <div key={idx}>
                    <div className="info-label">{p.name}</div>
                    <div className="info-value">
                      {p.qty} × ₹{p.unitPrice} = ₹{p.total}
                    </div>
                  </div>
                ))}
                ...
                <div>
                  ...
                  <div className="info-value">₹{invoiceData.amount}</div>
                </div>
              ...
              <div className="actions">
                <button className="btn-primary" onClick={() => generateInvoice(invoiceData)}>
                  Download Invoice
                </button>
